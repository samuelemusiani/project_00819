 ...
 *
 * vector.hpp: Personal implementation of
...
template <typename T> void vector<T>::reallocate(size_t new_capacity) {
  if (new_capacity >= 0 && this->_capacity != new_capacity) {
    ...
    // std::memcpy(tmp, this->_buffer, std::min(this->_capacity, new_capacity) *
    // sizeof(T));
    std::copy(this->_buffer,
              this->_buffer + std::min(this->_capacity, new_capacity), tmp);
...
template <typename T>
vector<T>::vector(int size) : _buffer(nullptr), _size(0), _capacity(0) {
  this->resize(size);
}
...
template <typename T> const T &vector<T>::at(const int pos) const {
  return *(this->_buffer + std::min(pos, (int)this->_size - 1));
}

template <typename T> size_t vector<T>::size() const { return this->_size; }
...
template <typename T> void vector<T>::resize(const int size) {
  if (size >= 0 && size != this->_size) {
    if (size < this->_size) {
      this->_size = size;

      if (this->_size >= this->_capacity / 4 &&
          this->_size <= this->_capacity / 3)
