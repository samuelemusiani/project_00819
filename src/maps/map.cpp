...
Chunk Map::get_chunk(int n) {
  ...
  if (n == 0) {
    ...
    c = this->possible_chunks[Random::generateChunk(
        this->seed, -n)]; // this shouldn't happen
  return c;
}

int Map::getCoins(int n) { return Random::generateCoins(this->seed, n); }

int Map::getEnemies(int n) { return Random::generateEnemies(this->seed, n); }

int Map::getSeed() { return this->seed; }
