...
double phy::Vector::get_magnitude() { return this->magnitude; }

int phy::Vector::get_direction() { return this->alpha; }
...
phy::Vector phy::Vector::sum(phy::Vector v1, phy::Vector v2) {
  ...
  v.set_magnitude(sqrt(mV1 * mV1 + mV2 * mV2 +
                       2 * mV1 * mV2 * cos(phy::PI / 180 * (aV1 - aV2))));
  v.set_direction(
      round(aV1 + atan2(mV2 * sin(phy::PI / 180 * (aV2 - aV1)),
                        mV1 + mV2 * cos(phy::PI / 180 * (aV2 - aV1))) *
                      180 / phy::PI));
