 ...
 *
 * credits.cpp: Class that draws the credits
...
int Credits::drawCredits(Draw *screen) {
  ...
  nostd::string creditstext[6] = {
      ...
      "in the year 2023 by",
      "Argonni Emanuele, Musiani Samuele",
      "Peronese Lorenzo, Ayache Omar"};
  ...
  for (int i = 42; i > -6 && !selected; i = i - 1) {
    ...
    for (int j = 0; j < 4; j++) {
      screen->drawText(10 + 2 * j, 20 - (developers[j].length() / 2),
                       developers[j]);
    }
    ...
    screen->drawText(10 + 2 * selectedOption,
                     20 - (developers[selectedOption].length() / 2),
                     developers[selectedOption]);
