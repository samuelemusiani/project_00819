...
private:
  static bool
  openFile(std::fstream &file, nostd::string path,
           nostd::string mode = "rw"); // mode can be r,w,rw,app; default is rw
  static bool exist(std::fstream &file, nostd::string path);
  static void appendSave(Map m, int chunk, phy::Point pos, Statistics stats,
                         nostd::string enitites, nostd::string market,
                         nostd::string name);
...
public:
  ...
  static void saveMap(Map m, int chunk, phy::Point pos, Statistics stats,
