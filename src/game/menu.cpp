 ...
 *
 * menu.cpp: Class that draw the main menu
 */
...
int Menu::get_selected_option(Draw *screen) {
  ...
  screen->nodel(false); // make getch() wait for input so that the menu doesn't
                        // refresh too fast - CPU FIX
  while (!isSelected) {
    // Scrive le opzioni del menu
    for (int i = 0; i < NUMBER_OF_OPTIONS; i++) {
      screen->drawText((posY / 2 - 2) + 2 * i,
                       posX / 2 - (options[i].length() / 2),
                       options[i].c_str());
    }

    screen->attrOn(COLOR_PAIR(1));
    screen->drawText((posY / 2 - 2) + 2 * selectedOption,
                     posX / 2 - (options[selectedOption].length() / 2),
                     options[selectedOption].c_str());
