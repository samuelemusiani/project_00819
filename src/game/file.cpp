...
bool File::openFile(std::fstream &file, nostd::string path,
                    nostd::string mode) {
  if (mode == "r")
...
bool File::isCalibrated(Settings &sett) { return sett.getCalibration() != -1; }
...
void File::appendSave(Map m, int chunk, phy::Point pos, Statistics stats,
                      nostd::string entities, nostd::string market,
                      nostd::string name) {
  std::fstream file;
  if (openFile(file, "./save.txt", "app")) {
    file << "[ Name: " << name << " ]\nSeed: " << m.getSeed()
         << "\nChunk: " << chunk << "\nPlayerPos: " << pos.get_xPosition()
...
void File::saveMap(Map m, int chunk, phy::Point pos, Statistics stats,
                   nostd::string entities, nostd::string market,
                   nostd::string name) {
  if (!isAlreadySaved(m)) {
    if (!name.empty())
      ...
      appendSave(m, chunk, pos, stats, entities, market,
                 "Player"); // this shouldn't happen
  } else
...
Statistics File::getStatistics(nostd::string name) {
  ...
  if (openFile(file, "./save.txt", "r")) {
    ...
    if (found) {
      ...
      stats.setLevel(nostd::stoi(
          buff.substr(pos_space[0] + 1, pos_space[1] - pos_space[0] - 1)));
      stats.setCoins(nostd::stoi(
          buff.substr(pos_space[1] + 1, pos_space[2] - pos_space[1] - 1)));
      stats.setJumps(nostd::stoi(
          buff.substr(pos_space[2] + 1, pos_space[3] - pos_space[2] - 1)));
      stats.setHearts(nostd::stoi(
          buff.substr(pos_space[3] + 1, pos_space[4] - pos_space[3] - 1)));
    }
  }
